...
| Agent Team | Multi-agent collaboration: a Leader plans and dispatches, Teammates execute one requirement in parallel | pending | [English](1-agentteam-demo/agentteam_readme.md) | pending | In progress |
| SDD | Spec-driven development: one business requirement becomes a spec, a design, tasks, and a verifiable page via `/sdd-new` → `/sdd-design` → `/sdd-tasks` → `/sdd-apply` | Two rounds of incremental requirements (ticket form + unique ticket number) on one static page | [Chinese](sdd/sdd_readme.zh-CN.md) | pending | Done |
