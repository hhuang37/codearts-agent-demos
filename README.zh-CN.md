...
| Agent Team | 多智能体协作：Leader 拆解分派，Teammate 并行执行同一条需求 | 待跑 | [中文](1-agentteam-demo/README.zh-CN.md) | 待录制 | 进行中 |
| SDD | 规范驱动开发：一句业务需求经 `/sdd-new` → `/sdd-design` → `/sdd-tasks` → `/sdd-apply` 变成 spec、设计、任务和可验证的页面 | 单个静态页面上完成两轮增量需求（工单登记 + 唯一工单编号） | [中文](sdd/行销文案.md) | 待录制 | 已完成 |
